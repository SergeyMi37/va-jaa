...
def start(core:VACore):
    manifest = {
        "name": "Погода в Кайзере (open weather map)",
        "version": "1.0",
        "require_online": True,

        "default_options": {
            "apiKey": "b14702632e0308563a36e160f366bd68", # get at https://openweathermap.org/
        ...
        "commands": {
            "кайзер погода": (run_weather, ""), # погода завтра, погода послезавтра
            "кайзер прогноз|кайзерляутерн прогноз": (run_weather, "prognoz"),
        }
    }
    return manifest
...
def run_weather(core:VACore, phrase:str, addparam:str):
    ...
    try:
        ...
        res = requests.get("https://api.openweathermap.org/data/2.5/onecall",
                           params={'lon': options["lon"], 'lat': options["lat"], 'units': 'metric', 'lang': 'de', 'APPID': options["apiKey"]})
